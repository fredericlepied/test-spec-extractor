...
def generate_quality_metrics(analysis):
    ...
    metrics.append(
        f"- Test Type Diversity: {coverage_metrics['test_type_diversity']} different types"
    )
    metrics.append(
        f"- Purpose Diversity: {coverage_metrics['purpose_diversity']} different purposes"
    )
    metrics.append(
        f"- Environment Diversity: {coverage_metrics['environment_diversity']} different environments"
    )
    metrics.append("")

    metrics.append("**Coverage Depth:**")
    metrics.append(
        f"- Operations per Test: {coverage_metrics['avg_operations_per_test']:.1f} average"
    )
    metrics.append(
...
def generate_insights_and_recommendations(analysis):
    ...
    if test_types:
        ...
        if (
            most_common_type[0] == "unit"
            and most_common_type[1] / analysis["total_tests"] > 0.8
        ):
            recommendations.append(
                "Consider adding more integration tests to improve end-to-end coverage"
            )
        elif (
            most_common_type[0] == "integration"
            and most_common_type[1] / analysis["total_tests"] > 0.8
        ):
            recommendations.append(
                "Consider adding more unit tests for better component-level coverage"
            )
    ...
    if purposes:
        ...
        if purpose_count < 3:
            recommendations.append(
                "Consider diversifying test purposes to cover more scenarios"
            )
        elif purpose_count > 8:
            recommendations.append(
                "Consider consolidating similar test purposes for better organization"
            )
    ...
    if environments:
        ...
        if env_count == 1:
            recommendations.append(
                "Consider testing across multiple environments for better coverage"
            )
        elif env_count > 5:
    ...
    if operations:
        ...
        if unique_ops / total_ops < 0.1:
            recommendations.append(
                "Consider diversifying operations to cover more Kubernetes APIs"
            )
        elif unique_ops / total_ops > 0.5:
            recommendations.append(
                "Consider consolidating similar operations for better maintainability"
            )
    ...
    if resources:
        ...
        if resource_count < 5:
            ...
            recommendations.append(
                "Consider focusing on core resource types for better maintainability"
            )
