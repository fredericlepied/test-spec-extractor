...
def rerank_pair(spec_a: Dict[str, Any], spec_b: Dict[str, Any], repo_a="A", repo_b="B"):
    ...
    user = USER_TMPL.format(repo_a=repo_a, repo_b=repo_b, spec_a=json.dumps(spec_a, ensure_ascii=False), spec_b=json.dumps(spec_b, ensure_ascii=False))
    ...
    for attempt in range(3):
        try:
            ...
            if overlap not in {"full","partial","none"}: overlap = "none"
...
def rerank_batch(pairs: List[dict], specs_a: List[Dict[str,Any]], specs_b: List[Dict[str,Any]]) -> List[dict]:
    out = []
    for p in pairs:
        i, j = p['idx_a'], p['idx_b']
        overlap, score, evidence, differences = rerank_pair(specs_a[i], specs_b[j], repo_a=specs_a[i].get('_repo','A'), repo_b=specs_b[j].get('_repo','B'))
        final = dict(p)
        final['llm_overlap'] = overlap
        final['llm_score'] = float(score)
        final['llm_evidence'] = evidence
        final['llm_diff'] = differences
        final['blended_score'] = 0.7*(float(score)/100.0) + 0.3*max(0.0, min(1.0, p.get('base_score', 0)))
        out.append(final)
    return out
