...
def generate_insights(analysis):
    ...
    if analysis['test_types']:
        ...
        insights.append(f"Primary test type: {most_common_type[0]} ({most_common_type[1]} tests, {most_common_type[1]/analysis['total_tests']*100:.1f}%)")
    
    # Purpose insights
    if analysis['purposes']:
        most_common_purpose = analysis['purposes'].most_common(1)[0]
        insights.append(f"Main focus: {most_common_purpose[0]} ({most_common_purpose[1]} tests, {most_common_purpose[1]/analysis['total_tests']*100:.1f}%)")
    ...
    if analysis['environments']:
        env_list = ", ".join([f"{env} ({count})" for env, count in analysis['environments'].most_common(3)])
        insights.append(f"Target environments: {env_list}")
    
    # Coverage insights
    if analysis['coverage_metrics']['unique_operations'] > 0:
        insights.append(f"Operation coverage: {analysis['coverage_metrics']['unique_operations']} unique operations across {analysis['coverage_metrics']['unique_resources']} resource types")
    
    # OpenShift insights
    if analysis['openshift_resources']:
        openshift_count = sum(analysis['openshift_resources'].values())
        insights.append(f"OpenShift-specific: {openshift_count} operations using {len(analysis['openshift_resources'])} OpenShift resources")
...
def generate_recommendations(comparison):
    ...
    if comparison['operation_gaps']['unique_to_suite1']:
        recommendations.append(f"Consider adding {len(comparison['operation_gaps']['unique_to_suite1'])} operations from {comparison['suite1']} to {comparison['suite2']}")
    
    if comparison['operation_gaps']['unique_to_suite2']:
        recommendations.append(f"Consider adding {len(comparison['operation_gaps']['unique_to_suite2'])} operations from {comparison['suite2']} to {comparison['suite1']}")
    
    # Resource recommendations
    if comparison['resource_gaps']['unique_to_suite1']:
        recommendations.append(f"Consider adding tests for {len(comparison['resource_gaps']['unique_to_suite1'])} resource types from {comparison['suite1']} to {comparison['suite2']}")
    
    if comparison['resource_gaps']['unique_to_suite2']:
        recommendations.append(f"Consider adding tests for {len(comparison['resource_gaps']['unique_to_suite2'])} resource types from {comparison['suite2']} to {comparison['suite1']}")
    
    # Purpose recommendations
    if comparison['purpose_gaps']['unique_to_suite1']:
        recommendations.append(f"Consider adding {len(comparison['purpose_gaps']['unique_to_suite1'])} purpose categories from {comparison['suite1']} to {comparison['suite2']}")
    
    if comparison['purpose_gaps']['unique_to_suite2']:
        recommendations.append(f"Consider adding {len(comparison['purpose_gaps']['unique_to_suite2'])} purpose categories from {comparison['suite2']} to {comparison['suite1']}")
    
    # Coverage recommendations
    if comparison['coverage_comparison']['suite1_operations'] > comparison['coverage_comparison']['suite2_operations']:
        recommendations.append(f"{comparison['suite1']} has {comparison['coverage_comparison']['suite1_operations'] - comparison['coverage_comparison']['suite2_operations']} more unique operations than {comparison['suite2']}")
    elif comparison['coverage_comparison']['suite2_operations'] > comparison['coverage_comparison']['suite1_operations']:
        recommendations.append(f"{comparison['suite2']} has {comparison['coverage_comparison']['suite2_operations'] - comparison['coverage_comparison']['suite1_operations']} more unique operations than {comparison['suite1']}")
...
def print_suite_comparison(comparison):
    ...
    print(f"  {comparison['suite1']} Operations: {comparison['coverage_comparison']['suite1_operations']}")
    print(f"  {comparison['suite2']} Operations: {comparison['coverage_comparison']['suite2_operations']}")
    print(f"  {comparison['suite1']} Resources: {comparison['coverage_comparison']['suite1_resources']}")
    print(f"  {comparison['suite2']} Resources: {comparison['coverage_comparison']['suite2_resources']}")
    
    print(f"\n🔍 OPERATION GAPS:")
    if comparison['operation_gaps']['unique_to_suite1']:
        print(f"  Unique to {comparison['suite1']} ({len(comparison['operation_gaps']['unique_to_suite1'])}):")
    ...
    if comparison['operation_gaps']['unique_to_suite2']:
        print(f"  Unique to {comparison['suite2']} ({len(comparison['operation_gaps']['unique_to_suite2'])}):")
    ...
    print(f"\n🎯 PURPOSE GAPS:")
    if comparison['purpose_gaps']['unique_to_suite1']:
        print(f"  Unique to {comparison['suite1']}: {', '.join(comparison['purpose_gaps']['unique_to_suite1'])}")
    if comparison['purpose_gaps']['unique_to_suite2']:
        print(f"  Unique to {comparison['suite2']}: {', '.join(comparison['purpose_gaps']['unique_to_suite2'])}")
    
    print(f"\n🌍 ENVIRONMENT GAPS:")
    if comparison['environment_gaps']['unique_to_suite1']:
        print(f"  Unique to {comparison['suite1']}: {', '.join(comparison['environment_gaps']['unique_to_suite1'])}")
    if comparison['environment_gaps']['unique_to_suite2']:
        print(f"  Unique to {comparison['suite2']}: {', '.join(comparison['environment_gaps']['unique_to_suite2'])}")
    ...
    if args.compare and len(all_analyses) > 1:
        ...
        for i in range(len(suite_names)):
            for j in range(i + 1, len(suite_names)):
                ...
                comparison = compare_test_suites(all_analyses[suite1_name], all_analyses[suite2_name])
