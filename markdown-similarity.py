...
class MarkdownSimilarityAnalyzer:
    ...
    def _parse_markdown_file(self, md_file: Path) -> List[TestContext]:
        ...
        # Simple state machine to parse the markdown with safety limits
        i = 0
        ...
        while i < len(lines) and iterations < max_iterations:
            ...
            # Extract test cases
                ...
                # Skip tests with empty descriptions
                if not test_desc:
                    continue
                test_labels = []
                test_prep = []
                ...
                while i < len(lines) and not lines[i].strip().startswith('- **Test**:'):
                    ...
                    if line.startswith('- labels:'):
                        test_labels = [l.strip() for l in line[9:].split(',')]
                    elif line.startswith('- preparation:'):
                        i += 1
                        while i < len(lines) and lines[i].strip().startswith('- '):
                            test_prep.append(lines[i].strip()[2:])
                            i += 1
                        i -= 1  # Back up one
                    elif line.startswith('- steps:'):
                        i += 1
                        while i < len(lines) and lines[i].strip().startswith('- '):
                            test_steps.append(lines[i].strip()[2:])
                            i += 1
                        i -= 1  # Back up one
                    elif line.startswith('- cleanup:'):
                        i += 1
                        while i < len(lines) and lines[i].strip().startswith('- '):
                            test_cleanup.append(lines[i].strip()[2:])
                            i += 1
                        i -= 1  # Back up one
        ...
        if iterations >= max_iterations:
            print(f"Warning: Hit safety limit parsing {md_file.name}, may have incomplete extraction")
    ...
    def find_similar_tests(self, query_documents: List[TestDocument], 
                          top_k: int = 10, similarity_threshold: float = 0.7, exclude_same_file: bool = True) -> List[Dict[str, Any]]:
        ...
        for i, (query_doc, doc_scores, doc_indices) in enumerate(zip(query_documents, scores, indices)):
            for j, (score, idx) in enumerate(zip(doc_scores, doc_indices)):
                ...
                if exclude_same_file and query_doc.context.file_path == matched_doc.context.file_path:
                    continue
                ...
                result = {
                    ...
                    'shared_labels': list(set(query_doc.context.test_labels) & 
                                         set(matched_doc.context.test_labels))
                }
...
def main():
    ...
    parser.add_argument('--repo-roots', nargs='*', default=[], 
                       help='Repository root directories for path normalization')
    parser.add_argument('--model', default='all-MiniLM-L6-v2', 
                       help='SentenceTransformer model name')
    parser.add_argument('--top-k', type=int, default=10, 
                       help='Top K similar tests to find')
    parser.add_argument('--threshold', type=float, default=0.7,
                       help='Similarity threshold (0.0-1.0)')
    parser.add_argument('--exclude-same-file', action='store_true', default=True,
                       help='Exclude matches within the same file (default: True)')
    parser.add_argument('--include-same-file', action='store_false', dest='exclude_same_file',
    ...
    if similarities:
        ...
        if args.repo_roots:
            ...
            df['query_file'] = df['query_file'].apply(lambda x: normalize_file_path(x, args.repo_roots))
            df['matched_file'] = df['matched_file'].apply(lambda x: normalize_file_path(x, args.repo_roots))
